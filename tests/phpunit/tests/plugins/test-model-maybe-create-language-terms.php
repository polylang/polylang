...
class Model_Maybe_Create_Language_Terms_Test extends PLL_UnitTestCase {
	...
	public function test_maybe_create_language_terms() {
		...
		$foo = new PLLTest_Translatable( self::factory()->pll_model );
		$tax = $foo->get_tax_language();
		self::factory()->pll_model->translatable_objects->register( $foo );

		// Languages we'll work with.
		self::factory()->language->create( array( 'locale' => 'es_ES' ) );
		self::factory()->language->create( array( 'locale' => 'de_DE' ) );

		// Get the term_ids to delete.
		$term_ids = array();
		foreach ( self::factory()->pll_model->get_languages_list() as $language ) {
		...
		self::factory()->pll_model->clean_languages_cache();
		$links_model = self::factory()->pll_model->get_links_model();
		...
		self::factory()->pll_model->maybe_create_language_terms();
