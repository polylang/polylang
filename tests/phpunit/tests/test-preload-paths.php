...
class Preload_Paths_Test extends PLL_UnitTestCase {
	...
	public function test_preload_paths_with_post_editor_context( $path, $is_filtered, $post, $language, $is_translatable ) {
		...
		$filtered_path = $this->admin_block_editor->filter_preload_paths( array( $path ), $this->get_context( 'core/edit-post', $post ) );
	...
	public function test_preload_path_for_translatable_media() {
		...
		$this->assertSameSets(
			$media_path['expected'],
			$this->admin_block_editor->filter_preload_paths( $media_path['raw'], $this->get_context( 'core/edit-post', $post ) ),
	 ...
	 * @param string|array $path The preload path under test. Could be an array if provided along a HTTP method.
	 */
	public function test_preaload_paths_in_site_editor_context( $path ) {
		$this->_test_unfiltered_context( $path, 'core/edit-site' );
	}
	 ...
	 * @param string|array $path The preload path under test. Could be an array if provided along a HTTP method.
	 */
	public function test_preaload_paths_in_widget_editor_context( $path ) {
		$this->_test_unfiltered_context( $path, 'core/edit-widgets' );
	}

	private function _test_unfiltered_context( $path, $context_name ) {
		$this->assertSameSets(
			array( $path ),
			$this->admin_block_editor->filter_preload_paths( array( $path ), $this->get_context( $context_name ) ),
			"Path should not be filtered in {$context_name} context."
		);
	}

	private function get_context( $context_name, $post = null ) {
