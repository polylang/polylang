...
test.describe( 'Check Post language on Block Editor', { tag: [ '@pre-release' ] }, async () => {
	let englishPost;
	/**
	 * Before all tests:
	...
	test( 'The block editor opens for the FR translation', async ( {
		...
		const href = await page
			.getByRole( 'link', { name: ' Edit the translation in Français' } )
			.getAttribute( 'href' );
		expect( href ).toMatch(
			new RegExp(
...
test.describe( 'Check Post language on Classic Editor', { tag: [ '@pre-release' ] }, async () => {
	let englishPost;
	/**
	 * Before all tests:
	 *     - Create en_US and fr_FR languages.
	 ...
	 * - The title field is visible and can be edited.
	 */
	test( 'The classic editor opens for the FR translation', async ( {
																		 admin,
																		 page,
																	 } ) => {
		await admin.visitAdminPage(
		...
		expect( href ).toMatch(
			new RegExp(
				`/wp-admin/post\\.php\\?post=${ frenchPostId }&action=edit`
			)
		);
	} );
} );
