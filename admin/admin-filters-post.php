...
class PLL_Admin_Filters_Post extends PLL_Admin_Filters_Post_Base {
	...
	public function parse_query( $query ) {
		...
		if ( isset( $qvars['post_type'] ) && ! isset( $qvars['lang'] ) ) {
			...
			} elseif ( isset( $_GET['post'] ) && $lang = $this->model->post->get_language( (int) $_GET['post'] ) ) {
				// Filters a 3rd party list of posts when editing a post ( needed for ACF )
				$query->set( 'lang', $lang->slug );
			} elseif ( isset( $_GET['new_lang'] ) && $lang = $this->model->get_language( $_GET['new_lang'] ) ) {
				// Filters a 3rd party list of posts when creating a new post ( needed for ACF )
				$query->set( 'lang', $lang->slug );
