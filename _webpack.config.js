...
function configureWebpack( options ){
	...
	const jsFileNamesEntries = [
		...jsFileNames.map( transformJsEntry( path.resolve( __dirname ) + '/build/js', true ) ),
		...jsFileNames.map( transformJsEntry( path.resolve( __dirname ) + '/build/js', false ) )
	]
	...
	const cssFileNamesEntries = cssFileNames.map( transformCssEntry( path.resolve( __dirname ) + '/build/css', isProduction ) );
