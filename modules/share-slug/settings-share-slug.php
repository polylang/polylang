		...
		<script type='text/javascript'>
			...
			( function( $ ){
				$( "input[name='force_lang']" ).change( function() {
					var value = $( this ).val();
					if ( value > 0 ) {
						$( "#pll-module-share-slugs" ).removeClass( "inactive" ).addClass( "active" ).children( "td" ).children( ".row-actions" ).html( '<?php echo esc_html( $activated ); // phpcs:ignore WordPress.Security.EscapeOutput ?>' );
					}
					else {
						$( "#pll-module-share-slugs" ).removeClass( "active" ).addClass( "inactive" ).children( "td" ).children( ".row-actions" ).html( '<?php echo esc_html( $deactivated ); // phpcs:ignore WordPress.Security.EscapeOutput ?>' );
					}
				} );
			} )( jQuery );
			// ]]>
